...
public class MemberServiceImpl implements MemberService {
	...
	//password Encode 하기 위한 secrity 디펜던스 추가.
	...
	public int register(MemberVO mvo) {
		...
		//아이디가 중복되면 회원가입 실패
		// 중복이 아니라면 아이디를 주고, DB 에서 일치하는 유저를 달라고 요청
		//일치하는 유저가 없다면 가입(1), 그렇지 않다면, 유저가 있으면 실패(0)
		...
		//password가 null 이면, 혹은 값이 없다면 ... 가입 불가
		...
		//회원가입 진행
		//암호화(encoding)//matches (원래 비번, 암호화된 비번)=> true/false
		String pw = mvo.getPw();
